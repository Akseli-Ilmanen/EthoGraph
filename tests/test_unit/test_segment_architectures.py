...
def test_every_registered_architecture_maps_to_an_upstream_config() -> None:
    """Every architecture is vendored, so every one reads its defaults from upstream's YAML."""
    ...
    assert sorted(_STEMS) == available_architectures()
