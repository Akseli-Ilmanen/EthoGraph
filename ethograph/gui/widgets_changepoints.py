...
"""TO DO: Refcator and unify some button functionality for 'detect' and 'clear' and 'interrupt' across modes."""

import numpy as np
import xarray as xr
from qtpy.QtWidgets import (
    QGridLayout, QLineEdit, QWidget, QPushButton,
    QVBoxLayout, QHBoxLayout, QLabel, QGroupBox, QCheckBox,
    QFrame, QComboBox, QDoubleSpinBox,
)
...
class RupturesWorker(QObject):
    """Worker for running ruptures detection in a background thread."""

    finished = Signal(object)  # Emits (bkps, error_msg)

    def __init__(self, signal, method, model, min_size, jump, params):
        super().__init__()
        self.signal = signal
        self.method = method
        self.model = model
    ...
    def run(self):
        ...
        try:
            ...
            if self.method == "Pelt":
                algo = rpt.Pelt(model=self.model, min_size=self.min_size, jump=self.jump).fit(self.signal)
                bkps = algo.predict(pen=self.params.get('penalty', 1.0))
            elif self.method == "Binseg":
                algo = rpt.Binseg(model=self.model, min_size=self.min_size, jump=self.jump).fit(self.signal)
                if self.params.get('penalty') is not None:
                    bkps = algo.predict(pen=self.params['penalty'])
                elif self.params.get('n_bkps') is not None:
                    bkps = algo.predict(n_bkps=self.params['n_bkps'])
                else:
                    bkps = algo.predict(n_bkps=5)
            elif self.method == "BottomUp":
                algo = rpt.BottomUp(model=self.model, min_size=self.min_size, jump=self.jump).fit(self.signal)
                bkps = algo.predict(n_bkps=self.params.get('n_bkps', 5))
            elif self.method == "Window":
                width = self.params.get('width', 100)
                algo = rpt.Window(width=width, model=self.model, min_size=self.min_size, jump=self.jump).fit(self.signal)
                bkps = algo.predict(n_bkps=self.params.get('n_bkps', 5))
            elif self.method == "Dynp":
                algo = rpt.Dynp(model=self.model, min_size=self.min_size, jump=self.jump).fit(self.signal)
                bkps = algo.predict(n_bkps=self.params.get('n_bkps', 5))
            else:
                self.finished.emit((None, f"Unknown method: {self.method}"))
                return
            ...
            self.finished.emit((bkps, None))

        except ImportError:
            self.finished.emit((None, "ruptures library not installed.\nInstall with: pip install ruptures"))
...
class ChangepointsWidget(QWidget):
    ...
    Toggle layout (like LabelsWidget):
      - Shared controls at top: Label changepoint correction, Show changepoints
      - Toggle buttons: Changepoints | Audio Changepoints
      - Content area shows one panel at a time
    """

    audio_changepoints_updated = Signal()
    request_plot_update = Signal()  # Emitted to request data_widget to refresh plot
    ...
    def __init__(self, napari_viewer: Viewer, app_state, parent=None):
        ...
        # Thread state for ruptures detection
        self._ruptures_thread = None
        self._ruptures_worker = None
        self._ruptures_context = None  # Stores (feature, time_coord_name, signal_len) for result handling
    ...
    def _create_shared_controls(self, main_layout):
        """Create shared controls visible for both panels."""
        ...
        self.changepoint_correction_checkbox = QCheckBox("Changepoint correction for labels")
    ...
    def _create_toggle_buttons(self, main_layout):
        """Create toggle buttons for switching between panels."""
    ...
    def _toggle_changepoints(self):
        """Toggle kinematic changepoints panel visibility."""
    ...
    def _toggle_audio_changepoints(self):
        """Toggle audio changepoints panel visibility."""
    ...
    def _toggle_ruptures(self):
        """Toggle ruptures panel visibility."""
    ...
    def _create_changepoints_panel(self):
        """Create panel for dataset changepoints."""
        ...
        self.prominence_edit.setToolTip("Minimum prominence of peaks (scipy.signal.find_peaks)")

        self.distance_edit = QLineEdit("2")
        self.distance_edit.setToolTip("Minimum horizontal distance between peaks in samples")
        ...
        self.threshold_edit.setToolTip("Gradient threshold for turning point detection (only for turning_points)")

        self.max_value_label = QLabel("Max value:")
        self.max_value_edit = QLineEdit("")
        self.max_value_edit.setToolTip("Maximum value to qualify as turning point (optional)")
    ...
    def _create_audio_changepoints_panel(self):
        """Create panel for audio changepoints."""
        ...
        self.compute_cp_button.setToolTip("Detect onset/offset candidates in current audio")
        ...
        ref_label = QLabel('<a href="https://github.com/timsainb/vocalization-segmentation" style="color: #87CEEB; text-decoration: none;">VocalSeg (Sainburg et al., 2020)</a>')
    ...
    def _create_ruptures_panel(self):
        """Create panel for ruptures changepoint detection."""
        ...
        self.ruptures_method_combo.currentTextChanged.connect(self._on_ruptures_method_changed)
        ...
        # Common parameters: min_size and jump
        ...
        self.ruptures_jump_edit.setToolTip("Subsampling factor (higher = faster but less precise)")
        ...
        # n_bkps (for Binseg, BottomUp, Window, Dynp)
        ...
        self.clear_ruptures_button.setToolTip("Remove all changepoints for current feature")
        ...
        ref_label = QLabel('<a href="https://centre-borelli.github.io/ruptures-docs" style="color: #87CEEB; text-decoration: none;">Ruptures (Truong et al., 2020)</a>')
        ref_label.setOpenExternalLinks(True)
    ...
    def _on_ruptures_method_changed(self, method: str):
        """Show/hide parameters based on selected ruptures method."""
        # Penalty: visible for Pelt, Binseg
        ...
        # n_bkps: visible for Binseg, BottomUp, Window, Dynp (not Pelt)
    ...
    def set_enabled_state(self, enabled: bool):
        """Enable/disable audio changepoints controls (based on audio availability)."""
    ...
    def _get_spectral_range(self) -> Optional[tuple]:
        """Get spectral range from spin boxes."""
    ...
    def _on_show_changepoints_changed(self, state):
        """Handle show changepoints checkbox change."""
    ...
    def _get_dataset_changepoint_indices(self):
        """Get changepoint indices from the current dataset, grouped by method.

        Returns:
            Tuple of (dict mapping method_name to indices, time_array) or (None, None)
        """
    ...
    def _compute_audio_changepoints(self):
        """Compute audio changepoints using vocalseg dynamic threshold segmentation."""
        ...
        try:
            ...
            silence_threshold = self._parse_float(self.silence_threshold_edit.text()) or 0.1
    ...
    def is_changepoint_correction_enabled(self) -> bool:
        """Check if changepoint correction is enabled."""
    ...
    def _clear_current_feature_changepoints(self):
        """Remove all changepoint variables for the current feature from the dataset."""
        ds = getattr(self.app_state, 'ds', None)
        if ds is None:
    ...
    def _compute_dataset_changepoints(self):
        """Compute changepoints for current feature and add to dataset."""
        ds = getattr(self.app_state, 'ds', None)
        if ds is None:
            show_warning("No dataset loaded")
            return
        ...
        # Validate that data is 1D after applying ds_kwargs
    ...
    def _clear_all_changepoints_for_feature(self, feature: str):
        """Remove all changepoint variables (kinematic + ruptures) for a feature."""
        ds = getattr(self.app_state, 'ds', None)
        if ds is None:
            return 0
    ...
    def _compute_ruptures_changepoints(self):
        """Compute changepoints using ruptures library in background thread."""
        ds = getattr(self.app_state, 'ds', None)
        if ds is None:
            show_warning("No dataset loaded")
            return
        ...
        # Store context for result handling
        ...
        self._ruptures_thread = QThread()
        self._ruptures_worker = RupturesWorker(signal, method, model, min_size, jump, params)
        self._ruptures_worker.moveToThread(self._ruptures_thread)

        self._ruptures_thread.started.connect(self._ruptures_worker.run)
        self._ruptures_worker.finished.connect(self._on_ruptures_finished)
        self._ruptures_worker.finished.connect(self._ruptures_thread.quit)
        self._ruptures_worker.finished.connect(self._ruptures_worker.deleteLater)
        self._ruptures_thread.finished.connect(self._ruptures_thread.deleteLater)

        self._ruptures_thread.start()

    def _cancel_ruptures_detection(self):
        """Cancel ongoing ruptures detection."""
        if self._ruptures_worker:
            self._ruptures_worker.cancel()
        if self._ruptures_thread and self._ruptures_thread.isRunning():
            self._ruptures_thread.quit()
            self._ruptures_thread.wait(1000)  # Wait up to 1 second
            if self._ruptures_thread.isRunning():
                self._ruptures_thread.terminate()

        self._reset_ruptures_ui()
        self.ruptures_count_label.setText("Cancelled")

    def _reset_ruptures_ui(self):
        """Reset ruptures UI after completion or cancellation."""
        self.cancel_ruptures_button.hide()
    ...
    def _on_ruptures_finished(self, result):
        """Handle ruptures detection completion."""
