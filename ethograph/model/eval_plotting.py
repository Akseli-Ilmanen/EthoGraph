...
def plot_metrics_best_model(
    ...
    # plt.savefig(path_pdf, format='pdf', dpi=600, bbox_inches='tight', pad_inches=0.05, facecolor='white', edgecolor='none')  # noqa: E501
    # plt.savefig(path,
    #             format='pdf',
...
def plot_overall_metrics_comparison(ax, nested_results_list: List[Dict[str, Any]], f1_thresholds: List[float]) -> None:
    ...
    ax.bar(
        ...
        label="Corrected",
        alpha=0.8,
        color=PRED_COLOURS[1],
    )

    # Plot individual points
    for i in x:
        ax.scatter(
            [i - width / 2] * len(uncorrected_points[i]),
            uncorrected_points[i],
            color=PRED_COLOURS[0],
            s=20,
            alpha=0.6,
            zorder=3,
        ...
        ax.scatter(
            [i + width / 2] * len(corrected_points[i]),
            corrected_points[i],
            color=PRED_COLOURS[1],
            s=20,
            alpha=0.6,
...
def plot_iou_visualization(ax, ious=[0.5, 0.75, 0.9]):
    ...
    for i, iou in enumerate(ious):
        idx = (gt_len + pred_len) * iou / (1.0 + iou)
    ...
    legend_elements = [Patch(facecolor="gray", label="Ground Truth")]
...
def plot_labelwise_results(ax, results_list: List[Dict], f1_thresholds: List[float], title: str) -> None:
    ...
    ax.bar(
        ...
        f1_means[:, 0],
        width,
        label=f"F1@{int(f1_thresholds[0] * 100)}",
        alpha=0.8,
        color=F1_COLOURS[0],
    )
    ax.bar(
        ...
        f1_means[:, 1],
        width,
        label=f"F1@{int(f1_thresholds[1] * 100)}",
        alpha=0.8,
        color=F1_COLOURS[1],
    )
    ax.bar(
        ...
        f1_means[:, 2],
        width,
        label=f"F1@{int(f1_thresholds[2] * 100)}",
        alpha=0.8,
        color=F1_COLOURS[2],
    )
...
def plot_metrics_over_epochs_on_axis(ax, result_paths: List[Path], f1_thresholds: List[float]) -> None:
    all_result_files = []
    ...
    ax.plot(unique_epochs, mean_metrics["Acc"], label="Accuracy", marker="o", color="black")
    ax.fill_between(unique_epochs, min_metrics["Acc"], max_metrics["Acc"], color="black", alpha=0.2)

    for i, threshold in enumerate(f1_thresholds):
        key = f"F1@{int(threshold * 100)}"
        ax.plot(unique_epochs, mean_metrics[key], label=key, marker="s", color=F1_COLOURS[i])
        ax.fill_between(
            unique_epochs,
            min_metrics[key],
            max_metrics[key],
...
def plot_combined_deltas_histogram(ax, nested_results_list: List[Dict[str, Any]], fps: float) -> None:
    ...
    all_uncorrected_start = np.concatenate([nr["uncorrected"]["start_deltas"] for nr in nested_results_list])
    all_uncorrected_end = np.concatenate([nr["uncorrected"]["end_deltas"] for nr in nested_results_list])
    all_corrected_start = np.concatenate([nr["corrected"]["start_deltas"] for nr in nested_results_list])
    all_corrected_end = np.concatenate([nr["corrected"]["end_deltas"] for nr in nested_results_list])
...
def plot_iou_distribution_with_tp_fp_fn(
    ax, nested_results_list: List[Dict[str, Any]], f1_thresholds: List[float]
) -> None:
    all_uncorrected_IoUs = np.concatenate([nr["uncorrected"]["all_IoUs"] for nr in nested_results_list])
    all_corrected_IoUs = np.concatenate([nr["corrected"]["all_IoUs"] for nr in nested_results_list])

    ax.hist(
        all_uncorrected_IoUs,
        bins=50,
        alpha=0.4,
        label="Uncorrected",
        color=PRED_COLOURS[0],
    )
    ax.hist(all_corrected_IoUs, bins=50, alpha=0.4, label="Corrected", color=PRED_COLOURS[1])
