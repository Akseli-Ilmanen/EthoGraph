...
def func_eval(ground_truth_dict, predictions_dict, video_list, f1_thresholds=[0.5, 0.75, 0.9]):
    """
...
def func_eval_labelwise(
    ...
    eval_classes = [
        c for c in np.sort(np.unique(np.concatenate(list(ground_truth_dict.values())))) if c not in bg_class
    ]
    ...
    for target_class in eval_classes:
        ...
        for vid in video_list:
            ...
            for s in range(len(f1_thresholds)):
                tp1, fp1, fn1, IoUs, starts_d, ends_d = f_score(
                    pred_masked, gt_masked, f1_thresholds[s], bg_class=bg_class
                )
                tp[s] += tp1
                fp[s] += fp1
                fn[s] += fn1
