...
def update_dt_with_matlab_pulse_onsets(
    ...
    # Load MATLAB data
    mat_data = loadmat(matlab_path, squeeze_me=True, struct_as_record=False)
    ...
    matlab_trials_dict = {trial.trial_num: trial for trial in all_trials}
...
def update_nc_with_matlab_trials(
    ...
    for node_name, node in dt.children.items():
        ...
        if event_data is not None:
            ...
            ds = ds.assign_coords(events=("events", ["disp_out", "disp_in", "box_in", "box_out"]))

        # Initialize labels
        n_time = len(ds.coords["time"])
        n_individuals = len(ds.coords["individuals"])
        ds["labels"] = (("time", "individuals"), np.zeros((n_time, n_individuals)))
