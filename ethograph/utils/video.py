...
# ) -> list[Path]:
#     """Export each continuous label segment from a labeled dataset as a separate .mp4 file.
...
#         label_ids: List of label IDs to export. If None, exports all non-zero labels.
...
#         ...     output_dir="clips/",
#         ...     label_ids=[2, 3],  # Only export labels 2 and 3
...
#     output_dir = Path(output_dir) if output_dir else video_path.parent / "label_clips"
...
#         segments = [(val, start, end) for val, start, end in segments if val in label_ids]
...
#     exported_paths = []
#     label_counters: dict[int, int] = {}

#     for label_id, start_idx, end_idx in segments:
#         label_counters[label_id] = label_counters.get(label_id, 0) + 1
#         count = label_counters[label_id]
...
#         output_name = f"label{label_id:02d}_{count:03d}_trial{trial}_f{start_frame}-{end_frame}{slowdown_suffix}.mp4"
...
# def export_label_clips_all_trials(
#     nc_path: str,
#     video_folder: str,
#     output_dir: Optional[str] = None,
#     individuals_sel: Optional[str] = None,
...
# ) -> list[Path]:
#     """Export label clips from all trials in a dataset.
...
#         label_ids: List of label IDs to export. If None, exports all.
#         padding_frames: Number of frames to add before/after each clip.
...
#     output_dir = Path(output_dir) if output_dir else nc_path.parent / "label_clips"
#     output_dir.mkdir(parents=True, exist_ok=True)
...
# def get_label_summary(nc_path: str, trial_sel: Optional[int] = None) -> dict:
#     """Get a summary of all label segments in a dataset.
...
#     Returns:
#         Dictionary with label statistics including segment counts and durations.
#     """
#     dt = TrialTree.open(str(nc_path))
#     label_dt = dt.get_label_dt()
...
#     summary: dict = {"trial": trial, "fps": fps, "data_sr": data_sr, "labels": {}}

#     for label_id, start_idx, end_idx in segments:
#         duration = (end_idx - start_idx) / data_sr

#         if label_id not in summary["labels"]:
#             summary["labels"][label_id] = {"count": 0, "total_duration": 0.0, "segments": []}

#         summary["labels"][label_id]["count"] += 1
#         summary["labels"][label_id]["total_duration"] += duration
#         summary["labels"][label_id]["segments"].append(
