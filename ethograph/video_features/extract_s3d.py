...
from ethograph.video_features.transforms import (
    CenterCrop,
    Resize,
...
class ExtractS3D(BaseExtractor):
    def __init__(self, args) -> None:
        ...
        self.transforms = torchvision.transforms.Compose(
            [ToFloatTensorInZeroOne(), Resize(224), CenterCrop((224, 224))]
        )
    ...
    def extract(self, video_path: str) -> Dict[str, np.ndarray]:
        ...
        black_frames_post = torch.zeros(
            num_black_frames_post,
            rgb.size(1),
            rgb.size(2),
            rgb.size(3),
            dtype=rgb.dtype,
    ...
    def load_model(self) -> Dict[str, torch.nn.Module]:
        ...
        s3d_kinetics400_weights_torch_path = (
            Path(__file__).resolve().parent / "checkpoint" / "S3D_kinetics400_torchified.pt"
        )
        if not s3d_kinetics400_weights_torch_path.exists():
            raise FileNotFoundError(f"S3D checkpoint not found at {s3d_kinetics400_weights_torch_path}")
