...
class AttLayer(nn.Module):
    ...
    def _block_wise_self_att(self, q, k, v, mask):
        ...
        if L % self.bl != 0:
            q = torch.cat(
                [q, torch.zeros((m_batchsize, c1, self.bl - L % self.bl)).to(q.device)],
                dim=-1,
            )
            k = torch.cat(
                [k, torch.zeros((m_batchsize, c2, self.bl - L % self.bl)).to(q.device)],
                dim=-1,
            )
            v = torch.cat(
                [v, torch.zeros((m_batchsize, c3, self.bl - L % self.bl)).to(q.device)],
                dim=-1,
            )
            nb += 1
        ...
        padding_mask = torch.cat(
            [
                torch.ones((m_batchsize, 1, L)).to(q.device) * mask[:, 0:1, :],
                torch.zeros((m_batchsize, 1, self.bl * nb - L)).to(q.device),
            ],
            dim=-1,
        )
    ...
    def _sliding_window_self_att(self, q, k, v, mask):
        ...
        k = torch.cat(
            [
                torch.zeros(m_batchsize, c2, self.bl // 2).to(q.device),
                k,
                torch.zeros(m_batchsize, c2, self.bl // 2).to(q.device),
            ],
            dim=-1,
        ...
        v = torch.cat(
            [
                torch.zeros(m_batchsize, c3, self.bl // 2).to(q.device),
                v,
                torch.zeros(m_batchsize, c3, self.bl // 2).to(q.device),
            ],
            dim=-1,
        ...
        padding_mask = torch.cat(
            [
                torch.zeros(m_batchsize, 1, self.bl // 2).to(q.device),
                padding_mask,
                torch.zeros(m_batchsize, 1, self.bl // 2).to(q.device),
            ],
            dim=-1,
        ...
        final_mask = self.window_mask.to(q.device).repeat(m_batchsize * nb, 1, 1) * padding_mask
