# Vendored from HaoyuJi/LaDy (MIT) — see NOTICE.md
# ruff: noqa
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.SP = MultiScale_GraphConv(13, in_channel, n_features, A_binary)
    ...
    def forward(self, x: torch.Tensor, q: torch.Tensor, mask: torch.Tensor, joint_graph) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if q.shape[0] != N:
            ...
            tau_hat = tau_hat.permute(0, 2, 1, 3).contiguous().view(N, T, -1)
            q_dot = q_dot.permute(0, 2, 1, 3).contiguous().view(N, T, -1)
