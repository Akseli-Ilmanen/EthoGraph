...
- The user drags a Media/Feature source from the left sidebar onto the plot area to create a panel; every panel has a ✕ button that removes it. Templates define layouts in the same instance terms.
- Initial visibility after a dataset load derives purely from data availability (`DataWidget._setup_panel_controls`): audio → audio trace + spectrogram, features → feature plot, neo/neurons → neural panels.
