...
ethograph/gui/
    ...
    dialog_label_frames.py    # Labels: Setup + Frames tabs — grid of frames at label times; ticked tiles seed the refine queue
...
- **Combining metadata filters have one implementation**: `allowed_trials_from_metadata(metadata_df, {column: values})` in `io/metadata_table.py` intersects the per-column filters and returns trial ids **as strings** (an empty value set means "any", and `None` comes back when nothing is filtered). Dialogs pair it with `CategoryFilterDialog` per condition column — never a single-column combo. Covered by `tests/test_unit/test_onset_dialog.py`.
...
- **Frame-by-frame refinement treats existing labels as seeds** (`dialog_refine.py`, Tools ▸ Refine labels frame-by-frame…). The dialog walks a queue of boundary seeds sorted (trial, onset); each is reached via `NavigationWidget.jump_to_label_instance(...)`; Enter commits `video.frame_to_time(current_frame)` → `from_display`. Enter is an ApplicationShortcut installed on `_start`, removed on `_stop`; no button is default/autoDefault. Backspace deletes the current event outright (whole row, both boundaries) and moves on. A queue can also be handed over from the frames grid (`start_from_seeds`, `targets_from_seeds`) — those seeds are the whole queue and are not written to `refine_resume`. Remembered per dataset (`refine_log` + `refine_resume`, SCOPE_LOCAL), exportable as `_prerefined.tsv`/`_postrefined.tsv`. The refine view is `refine_window_s` (SCOPE_GLOBAL). Covered by `tests/test_unit/test_refine_labels.py`.
- **Every label carries a `confidence`**: `HUMAN_CONFIDENCE` (1.0) for a hand-placed one, the producing model's own score otherwise. It is part of `INTERVAL_COLUMNS`, so it survives every per-trial round trip; `ensure_confidence` fills it in for files written without it. `dialog_label_frames.py` prints it per tile, outlines tiles below its threshold in red, and plots its distribution per label class (per individual too, when more than one is labelled) in a `ConfidenceHistogramsDialog` sharing that threshold. **A boundary confirmed in `dialog_refine.py` is stamped back to `HUMAN_CONFIDENCE`** — reviewing by eye is hand labelling. Covered by `tests/test_unit/test_label_intervals.py` (`TestConfidence`) + `tests/test_unit/test_label_frames.py` (`TestConfidenceFlagging`, `TestConfidenceGroups`).
