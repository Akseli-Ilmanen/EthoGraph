...
ethograph/utils/system_check.py # Linux preflight: the system libs the GUI wheels dlopen + the apt/dnf line (`ethograph check`, warned at launch)
...
Code-first, never in the GUI, and **scripted — there is no CLI**. One YAML config becomes a `Project`, and every stage is a method on it (`video_features`, `materialise`, `train`, `search`, `cross_validate`, `infer`, `compare`, `rank_video_features`); overrides are dotted `key=value` strings passed to `Project(...)`/`.update(...)`. One way to express a run, and it is a file you can diff. Vocabulary in `CONTEXT.md`; design in `docs/add_to_docs_later/segment/`; decisions in `docs/adr/`.
...
- **A click snaps to what the clicked panel draws.** `features/changepoints.changepoint_fired` is the one reading of a mask (a feature's masks at the panel's keypoint / individual selections, free dims OR'd): `XarrayLoader.select` draws it and `XarrayLoader.get_cp_times` snaps to it, so drawn and snapped are one set. `LabelsWidget._snap_candidates` picks the set by the panel in `click_info["plot"]` (audio panels → audio changepoints); the search is unbounded. A double-click while `app_state.label_drawing_armed` is a closing boundary, never an autoscale. `snap_boundaries` moves boundaries only — `confidence` and `labeling_method` survive. Covered by `tests/test_unit/test_changepoint_snap.py` + `test_label_dblclick.py`.
