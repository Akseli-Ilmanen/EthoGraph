...
**Pynapple trial resolution** (`_load_pynapple_dataset`): the alignment NWB's **trials table is the base truth** when it has `start_time` — `trials_ep` is built from it (real `trial` ids carried through `SourceCollection`/`app_state.trials`, not 1..N), and a user **tabular** metadata file merges into it via `merge_trial_metadata` (`io/metadata_table.py`): pynapple `start`/`end` spellings normalise to `start_time`/`stop_time` (`PYNAPPLE_TO_NWB_COLUMNS`), unknown/duplicate trial ids raise, columns only one side has are unioned, and shared columns with differing content become `MetadataConflict`s settled by a per-column dialog (`DataWidget._resolve_metadata_conflicts`, threaded through `load_features_dataset(metadata_conflict_resolver=…)`; headless default keeps alignment values). Only without an alignment trials table does the old chain apply: explicit metadata timing → IntervalSet detected in the data. Resolution priority (`_resolve_alignment`): explicit `alignment_path` wins → NWB sources use source NWB trials → sidecar TSV; non-NWB use sidecar `alignment.nwb` → sidecar TSV. `align_media_per_trial` / `align_media_from_streams` build ImageSeries from `stream_rates: dict[str, float]`.
