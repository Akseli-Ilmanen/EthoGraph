...
`PanelStateMixin._sanitize_selections` guarantees **at most one free multi-value dim**, and `set_panel_control` re-applies it on *every* change (not just a feature swap) — setting a dim to "All" is the other way a panel reaches two free dims and crashes the next `update_plot()`.
...
**The console rebinds on `plot_container.panel_content_changed`, NOT `active_changed`** — the latter fires only when the active panel *changes*, so re-clicking the current panel or swapping its feature in the sidebar left the console stale. Emitted from every feature panel's `plot_clicked`, from `DataWidget.apply_panel_control`, and from `MetaWidget._activate_panel` (a just-created panel has no click behind it). **That signal is the console's ONLY rebind channel** — `_on_active_panel` must never call `bind_panel` as well, or a click that also changes the active panel binds twice.
