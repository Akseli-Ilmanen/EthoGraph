...
ethograph/spot/               # Pixel point-event spotting (E2E-Spot); docs: docs/add_to_docs_later/spot/
    ...
    stream.py                 # Inference decodes the video straight into the model (rolling one-window buffer, JPEG round trip in memory); never the frame folder
...
- **The models and the loss are DLC2Action's, vendored in upstream's layout**, never edited beyond `NOTICE.md`. **Every architecture speaks one contract**: `model(x (B,F,T), mask (B,1,T)) → logits (S,B,C,T)`, `logits[-1]` is the prediction; extra heads ride a `ModelOutput` read through `as_output(...)`. **A default is never written in our code** — read from `dlc2action/config/` via `DLC2ACTION_CONFIG`. Covered by `tests/test_unit/test_segment_architectures.py` + `test_segment_losses.py`.
- **Every boundary setting is a duration, resolved against the dataset's own rate.** Covered by `tests/test_unit/test_segment_boundary.py`.
...
- **Every temporal setting is a duration, resolved against the video's own rate.** `ClipConfig(context_s, resolution_ms, positive_window_ms).resolve(fps)` is the one place upstream's `stride` / `clip_len` / `dilate_len` are computed, and `ResolvedClip.to_frame` maps a strided prediction back to the **centre** of its bin (`bin*k + (k-1)/2`). Upstream's frame counts were tuned at 25 fps and mean something else at 200. A combination exceeding `MAX_FRAMES_PER_BATCH` is refused naming the *duration* to change. Covered by `tests/test_unit/test_spot_config.py`.
- **A prediction's `confidence` is its curve's shape, not its peak height** (`confidence.py`: `focus × ratio`; a rival is another local maximum outside the window, never the peak's own shoulder). Measured on a held-out session: peak height separates a >50 ms error at AUC 0.58, the shape statistics at ~0.8 — a tie among them — and `ratio` is the one that is bimodal, so the histogram has a gap to threshold in; for the onset model peak height wins because its curve is shape-constrained by construction. The statistic is per model; that it stays readable off the drawn curve is not.
