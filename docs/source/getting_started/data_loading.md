...
~/.ethograph/                          # Global user defaults 
    ...
    └── gui_settings.yaml              # Across-session GUI state (changepoint settings, axes settings, ...)

my_project/
    ├── session.nc                     # Behavioural dataset (xarray)
    ├── .ethograph/
    │   └── alignment.nwb              # Media paths, trial timing, stream offsets
    │   └── local_settings.yaml        # Session-specific GUI state (feature selection, last paths, ...)
    │
    ├── labels/                        # Label files (created by GUI)
    │   ├── session_labels.tsv         # Most uptodate version in root folder
    │   └── backups/                  
